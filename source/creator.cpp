...
void InitCreator(py::module_ &m)
{
    ...
    py::class_<Operon::BalancedTreeCreator, Operon::CreatorBase>(m, "BalancedTreeCreator")
        .def(py::init([](Operon::PrimitiveSet const& grammar, std::vector<Operon::Variable> const& variables, double bias) {
            return Operon::BalancedTreeCreator(grammar, Operon::Span<Operon::Variable const>(variables.data(), variables.size()), bias);
    ...
    py::class_<Operon::ProbabilisticTreeCreator, Operon::CreatorBase>(m, "ProbabilisticTreeCreator")
        .def(py::init([](Operon::PrimitiveSet const& grammar, std::vector<Operon::Variable> const& variables, double bias) {
            return Operon::ProbabilisticTreeCreator(grammar, Operon::Span<Operon::Variable const>(variables.data(), variables.size()), bias);
        }), py::arg("grammar"), py::arg("variables"), py::arg("bias"))
    ...
    py::class_<Operon::GrowTreeCreator, Operon::CreatorBase>(m, "GrowTreeCreator")
        .def(py::init([](Operon::PrimitiveSet const& grammar, std::vector<Operon::Variable> const& variables) {
            return Operon::GrowTreeCreator(grammar, Operon::Span<Operon::Variable const>(variables.data(), variables.size()));
